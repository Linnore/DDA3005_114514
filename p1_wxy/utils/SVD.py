...
import sys
print(sys.setrecursionlimit(4500))
...
def svd_phaseIIB(B: np.ndarray, Qt: np.ndarray, P: np.ndarray, eigen=eigh_by_QR, tol=1e-15) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """This function implement the phaseII of SVD following the proposed procedure A in project description.
...
if __name__ == "__main__":
    ...
    #np.savetxt("goddamnit.txt", A)
    #A = np.loadtxt("goddamnit.txt")
    
    #A = np.array([[1, 0, 1], [2, 5**.5, 0], [0, 0, 1], [0, 0, 1]])
    #print(A)
    B, Qt, P = svd_phaseI(A)
    #U, T, Vt = svd_phaseIIA(B, Qt, P, eigen=eigh_by_QR)
    #print(A - U @ np.diag(T) @ Vt)
    t = time()
    U, T, Vt = svd_phaseIIB(B, Qt, P, eigen=eigh_by_QR_partB)
    print(A - U @ np.diag(T) @ Vt)
    print(time()-t)